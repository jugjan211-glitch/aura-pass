import { Shield, Sun, Moon, Zap, LogIn, Menu, X, Palette, User, LogOut, Lock, ShieldCheck } from 'lucide-react';
            ...
            <nav className="hidden md:flex items-center gap-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-40">
                  ...
                  <DropdownMenuItem onClick={() => setTheme('amoled')} className="gap-2"><Zap className="h-4 w-4" /> AMOLED</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <div className="container mx-auto px-4 py-4 space-y-3">
              <div className="flex gap-2">
                {(['light', 'dark', 'amoled'] as Theme[]).map(t => (
                  <Button key={t} variant={theme === t ? 'secondary' : 'ghost'} size="sm"
                    onClick={() => setTheme(t)} className="flex-1 capitalize">
                    {themeIcon[t]} <span className="ml-1">{t}</span>
                  </Button>
                ))}
              </div>
              <Button variant="outline" className="w-full gap-2" onClick={() => { setIsCustomizing(true); setMobileMenuOpen(false); }}>
                <Palette className="h-4 w-4" /> Customize
              </Button>
              {user ? (
                <div className="space-y-2">
                  <div className="text-sm text-muted-foreground text-center truncate px-2">{user.email}</div>
                  <Button variant="outline" className="w-full gap-2" onClick={() => { setTwoFactorOpen(true); setMobileMenuOpen(false); }}>
                    <ShieldCheck className="h-4 w-4" /> 2FA Settings
                  </Button>
                  <div className="flex gap-2">
                    <Button variant="outline" className="flex-1 gap-2" onClick={() => { handleLock(); setMobileMenuOpen(false); }}>
                      <Lock className="h-4 w-4" /> Lock
                    </Button>
                    <Button variant="destructive" className="flex-1 gap-2" onClick={() => { handleSignOut(); setMobileMenuOpen(false); }}>
                      <LogOut className="h-4 w-4" /> Sign Out
                    </Button>
                  </div>
                </div>
              ) : (
                <Button variant="default" className="w-full gap-2" onClick={() => { setAuthModalOpen(true); setMobileMenuOpen(false); }}>
                  <LogIn className="h-4 w-4" /> Sign In
                </Button>
              )}
            </div>
